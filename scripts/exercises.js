...
[t([],[','])],
['CHAR holds exactly one character', "Use single quotes for a CHAR literal: ','", 'DECLARE Separator : CHAR']),
...
[t([],['1500'])],
['Use CONSTANT MAX_TRANSMISSION_UNIT = 1500', 'OUTPUT the constant name directly']),

ex('bas-06','basics','REAL Constant: Tax Rate','easy',
`Declare a CONSTANT called \`TAX_RATE\` with value \`0.2\` (20% VAT). OUTPUT it.`,
[t([],['0.2'])],
['Use CONSTANT TAX_RATE = 0.2', 'OUTPUT the constant by name']),
...
[t([],['2073600'])],
['Declare both on one line: DECLARE ScreenWidth, ScreenHeight : INTEGER', 'OUTPUT ScreenWidth * ScreenHeight']),

ex('bas-09','basics','Input a Port Number','easy',
`INPUT an integer \`Port\` (a network port number). OUTPUT \`Port: \` followed by the port number.`,
[t(['8080'],['Port: 8080']), t(['443'],['Port: 443'])],
['Use OUTPUT with a comma to combine the label "Port: " and the Port variable']),
...
['Use NUM_TO_STR to convert each integer to a string', 'Join "v" & NUM_TO_STR(Major) & "." & NUM_TO_STR(Minor)']),
...
[t([],['Packets sent: 42'])],
['NUM_TO_STR(42) produces the string "42"', 'Join "Packets sent: " & NUM_TO_STR(PacketsSent)']),
...
['Use NUM_TO_STR(Major) and NUM_TO_STR(Minor) to convert each integer', 'Join "Version: " & NUM_TO_STR(Major) & "." & NUM_TO_STR(Minor)']),
...
[t([],['0','1','2','3','4','5','6','7'])],
['FOR Position ← 0 TO 7', 'Output Position on each iteration']),
...
[t([],['16','32','48','64','80','96'])],
['FOR Counter ← 1 TO 6', 'OUTPUT Counter * 16 on each iteration']),
...
['Initialise TotalBytes ← 0 before the loop', 'Add each input to TotalBytes on each iteration']),
...
['FOR Code ← Start TO Start + 4', 'OUTPUT Code on each iteration']),
...
['WHILE Retries < 3 (no DO keyword in AS 9618)', 'Build the output string using & and NUM_TO_STR(Retries + 1)', 'Increment Retries inside the loop body']),
...
['Initialise ErrorCount ← 0 before the loop', 'INPUT the first value before the WHILE, then check inside the loop']),
...
['REPEAT ... UNTIL LENGTH(Password) >= 8', 'The loop body always runs at least once']),
...
['Initialise Factorial ← 1 before the loop', 'Multiply Factorial by each counter from 1 to Number']),
...
['Initialise BitCount ← 0', 'WHILE Byte > 0: check Byte MOD 2, then Byte ← Byte DIV 2']),
...
['Initialise Count ← 0 before the loop', 'Use WHILE Total < 100 to control the loop']),
...
['FOR Celsius ← 0 TO 50 STEP 10', 'OUTPUT Celsius * 9 / 5 + 32 on each iteration']),
...
['The Euclidean algorithm: repeatedly replace (a, b) with (b, a MOD b) until b = 0', 'Use WHILE Num2 <> 0']),
...
['Use MID to extract the first character of each name', 'Apply UCASE to capitalise', 'Join: UCASE(MID(First,1,1)) & "." & UCASE(MID(Last,1,1)) & "."']),

ex('str-09','strings','Reverse a Three-Letter Word','medium',
`INPUT a 3-letter string into \`Word\`. OUTPUT the word reversed using MID.`,
[t(['Cat'],['taC']), t(['dog'],['god'])],
['Extract each character with MID(Word,3,1), MID(Word,2,1), MID(Word,1,1)', 'Concatenate them in reverse order using &']),
...
[t(['*','5'],['*****']), t(['X','3'],['XXX'])],
['Initialise Result ← ""', 'Use a FOR loop, concatenating Ch to Result on each iteration']),
...
['Use LENGTH, UCASE, and LCASE(MID(Sentence, 1, 1)) for the three outputs']),
...
[t([],['Hello!'])],
['Define with PROCEDURE Greet() and close with ENDPROCEDURE', 'Call with CALL Greet()'],
PROC_SCAFFOLD),
...
['Use OUTPUT with comma-separated values inside the procedure'],
PROC_SCAFFOLD),
...
[t([],['10'])],
['Inside the procedure: Num ← Num * 2', 'BYREF means the change persists in the caller'],
PROC_SCAFFOLD),
...
[t([],['49'])],
['The function body has a single RETURN statement: RETURN Num * Num'],
PROC_SCAFFOLD),
...
['Initialise Result ← 1 inside the function', 'Multiply Result by Base Exponent times in a FOR loop'],
PROC_SCAFFOLD),
...
['BYREF IsValid lets the procedure set the caller\'s variable', 'Inside: IsValid ← (Score >= 0 AND Score <= 100)'],
PROC_SCAFFOLD,
'',
requireBYREF),
...
['WHILE Num > 0: add Num MOD 10 to the sum, then Num ← Num DIV 10'],
PROC_SCAFFOLD),
...
[t([],['ASASAS'])],
['Initialise Result ← "" inside the function', 'Use a FOR loop to concatenate Str to Result Times times'],
PROC_SCAFFOLD),
...
[t([],['5'])],
['Initialise Guess ← SideA inside the function', 'Apply the iterative update 20 times in a FOR loop'],
PROC_SCAFFOLD),
...
[t([],['123'])],
['Loop through each character of NumStr', 'Convert each digit with STR_TO_NUM(MID(NumStr, Index, 1))', 'Accumulate: Result ← Result * 10 + digitValue'],
PROC_SCAFFOLD),
...
[t([],['10','20','30','40','50'])],
['Use Scores[1] ← 10, Scores[2] ← 20, etc.', 'Use a FOR loop to output all elements']),
...
[t([],['20','30','40','50','0'])],
['FOR i ← 1 TO 4: Data[i] ← Data[i+1]', 'After the loop: Data[5] ← 0']),
...
[t([],['5','1','2','3','4'])],
['Save Data[5] in Temp', 'Shift right: FOR i ← 5 TO 2 STEP -1, Data[i] ← Data[i-1]', 'Set Data[1] ← Temp']),
...
['Low ← 1, High ← 8, Position ← -1', 'WHILE Low <= High: Mid ← (Low + High) DIV 2; compare Data[Mid] to Target; update Low or High']),
...
`Define a \`PersonRecord\` type with \`Name : STRING\` and \`Score : INTEGER\`. INPUT a name and score. Store in record variable \`Person\`. OUTPUT both fields.`,
[t(['Bob','92'],['Bob','92'])],
...
[t([],['Clicks','3'])],
['Counter.Value ← Counter.Value + 1 three times', 'Two separate OUTPUT statements']),
...
[t([],['Bob'])],
['Initialise BestName ← Class[1].Name, BestMark ← Class[1].Mark', 'Compare each Class[i].Mark in a FOR loop; update when higher']),
...
[t([],['100','15'])],
['BYREF Score means the procedure modifies the original record', 'Inside: Score.Bonus ← Score.Bonus + Extra'],
PROC_SCAFFOLD,
'',
requireBYREF),
...
[t([],['Alice','01234'])],
['Copy fields individually: Contact2.Name ← Contact1.Name', 'Records cannot be assigned as a whole in Cambridge pseudocode']),
...
[t([],['RED','GREEN','BLUE'])],
['WHILE NOT EOF("colours.txt") — no DO keyword in AS 9618', 'Apply UCASE to each line read before outputting']),
...
[t([],['Alice','85'])],
['Write "Alice,85" as one line', 'Name: LEFT(Record, 5) or MID(Record, 1, 5)', 'Score: MID(Record, 7, 2)']),
...
[t([],['30'])],
['FOR Value ← 10 TO 50 STEP 10 to write', 'WHILE NOT EOF to read and accumulate the average'],
'', '',
...
[t([],['Charlie','92','Pass'])],
['Name: MID(Line, 1, 7)', 'Score: MID(Line, 9, 2)', 'Result: MID(Line, 12, 4)']),
...
[t([],['2','4','6','8','10'])],
['FOR Number ← 2 TO 10 STEP 2', 'WHILE NOT EOF — no DO keyword in AS pseudocode']),
...
['For "Alice,85": name is LEFT(Line, 5), score is MID(Line, 7, 2)', 'Output with & ": " & between them']),
...
['Low ← 1, High ← 8, Position ← -1', 'WHILE Low <= High: Mid ← (Low + High) DIV 2; compare; narrow range']),
...
['Track both the maximum value and its position', 'Update both when a new maximum is found']),
...
[t(['23'],['6']), t(['2'],['1']), t(['50'],['-1'])],
['Low ← 1, High ← 8; use WHILE Low <= High inside the function', 'Return Position after the loop'],
PROC_SCAFFOLD,
...
['"Computer Science" starts at position 4 and is 16 characters long', 'MID(FullText, 4, 16)', 'UCASE(FullText) for the uppercase output']),
...
[5 marks]`,
[t([],['3'])],
['BYREF means changes inside affect the original variable', 'Call CALL Increment(CallCount) three times'],
PROC_SCAFFOLD,
'',
requireBYREF),
...
[6 marks]`,
[t([],['Alice','Bob'])],
['Write each CSV string as a single WRITEFILE call', 'For "Alice,85" use LEFT(Line, 5); for "Bob,72" use LEFT(Line, 3)', 'WHILE NOT EOF — no DO keyword']),
...
[5 marks]`,
[t([],['BA001','London','180'])],
['Use TYPE FlightRecord ... ENDTYPE', 'Access fields with Flight.FlightCode, Flight.Destination, Flight.Seats']),
