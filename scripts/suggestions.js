...
// Super-admin (role: 'superadmin') sees all submissions and can reply.
...
export function initSuggestions(user, profile) {
  ...
  // Populate challenge selector in error modal
...
async function _loadInbox(isAdmin, filter = 'all') {
  ...
  try {
    // Fetch — sort client-side to avoid needing composite indexes
    let snapDocs;
    if (!isAdmin) {
      const q  = query(collection(db, 'suggestions'), where('userId', '==', _user.uid));
